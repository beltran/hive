...
// don't go  through Initiator for user initiated compactions)
  ...
  public void run() {
    ...
    do {
      ...
      try {
         ...
         */
        long compactorTxnId = msc.openTxns(runAs, 1).getTxn_ids().get(0);
        ...
        heartbeater = new CompactionHeartbeater(msc, compactorTxnId, fullTableName, conf);
        heartbeater.start();
        ...
        msc.setCompactionHighestWriteid(CompactionInfo.compactionInfoToStruct(ci),
        ...
        final StatsUpdater su = StatsUpdater.init(ci, msc.findColumnsWithStats(
            CompactionInfo.compactionInfoToStruct(ci)),
            conf, runJobAsSelf(runAs) ? runAs : t.getOwner());
  ...
  static final class CompactionHeartbeater extends Thread {
    ...
    public CompactionHeartbeater(IMetaStoreClient msc, long compactorTxnId, String tableName, HiveConf conf) {
      this.msc = msc;
